...
import { useTripInfoSectionsStore } from './trip-info-sections.store'

export enum ETripInfoKeys {
  FETCH_TRIP_DETAILS = 'trip:fetch-details',
...
export const useTripInfoStore = defineStore('tripInfo', {
  ...
  getters: {
    isLoading: () => useRequestStatus(ETripInfoKeys.FETCH_TRIP_DETAILS).value,
    fetchError: () => useRequestError(ETripInfoKeys.FETCH_TRIP_DETAILS).value,
  ...
  actions: {
    ...
    fetchTripDetails(tripId: string, initialDayIdFromQuery?: string) {
      this.currentTripId = tripId
      const sectionsStore = useTripInfoSectionsStore()

      useRequest({
        key: ETripInfoKeys.FETCH_TRIP_DETAILS,
        abortOnUnmount: true,
        ...
        onSuccess: (result) => {
          ...
          sectionsStore.setSections(result.sections || []) // Передаем разделы в их стор
